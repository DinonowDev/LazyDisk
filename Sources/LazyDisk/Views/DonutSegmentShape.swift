...
struct DonutSegmentShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        guard outerRadius > innerRadius + 1, endAngle > startAngle else { return path }
        ...
        path.addQuadCurve(to: polar(center, outerRadius, outerArcStart), control: polar(center, outerRadius, sa))
        ...
        path.addLine(to: polar(center, innerRadius, ea))
...
struct SegmentLayout: Identifiable {
    let id: UUID
    let item: DiskItem
    let index: Int
